...
export default function SessionControlScreen({
  ...
  /* calcular duración total base */
  useEffect(() => {
    const min = duration === "corto" ? 4 : duration === "mediano" ? 15 : 20
    setTotal(min * 60)
  }, [duration])
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-4">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <Button onClick={onEndSession} variant="outline" className="bg-gray-800 border-gray-700 text-white">
          ...
          <div className="flex items-center gap-2">
            ...
            {therapy.hasVideo && (
              <Badge className="bg-red-900/40 text-red-400 border-red-600/30">VIDEO</Badge>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <Card className="bg-gray-800 border-gray-700">
              ...
              <CardContent className="space-y-4">
                ...
                >
                  ...
                  {micEnabled ? "Apagar Micrófono" : "Encender Micrófono"}
                </Button>

                {/* Info */}
                <div className="text-xs bg-gray-700/30 p-2 rounded text-gray-400 space-y-1 mt-2">
                  <div className="flex justify-between">
                    <span>Estado:</span>
                    <span className={active ? "text-green-400" : "text-gray-400"}>
                      {active ? (paused ? "Pausada" : "Activa") : "Inactiva"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Micrófono:</span>
                    <span className={micReady ? "text-green-400" : "text-red-400"}>{micReady ? "ON" : "OFF"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Restante:</span>
                    <span>{fmt(Math.max(0, total - elapsed))}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Total:</span>
            ...
            <Card className="bg-gray-800 border-gray-700">
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between text-sm text-gray-300">
                  ...
                  <Badge className="bg-cyan-600/20 text-cyan-400">{lightIntensity}%</Badge>
                </div>
                <Slider
            ...
            {/* Reproductor de audio sólo cuando NO es video */}
            {!therapy.hasVideo && (
              <AudioPlayer
                sessionActive={active && !paused}
                sessionDuration={duration}
