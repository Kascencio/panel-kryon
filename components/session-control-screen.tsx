...
export interface Therapy {
  ...
  hasVideo?: boolean
  /* personalizados */
  lightMode?: string     // patrón de luz
}
...
export default function SessionControlScreen({
  ...
  useMicrophone(micOn)
  const { conectarArduino, iniciarTerapia, cambiarIntensidad, detenerTerapia } =
    useArduinoService()
  ...
  /* iniciar */
  const start = async () => {
    if (!(await conectarArduino())) return
    const mins = minutesOf(duration)
    const pattern = therapy.lightMode ?? therapy.frequency // ★ patrón de luz
    if (
      !(await iniciarTerapia(therapy.id, pattern, mins, lightIntensity))
    )
      return
    setActive(true)
    setPaused(false)
  ...
  const changeIntensity = async (v: number[]) => {
    const n = v[0]
          ...
          <div className="space-y-4">
            <Card className="bg-gray-800 border-gray-700">
              ...
              <CardContent className="space-y-4">
                ...
                {/* audio (si no es video) */}
                {!therapy.hasVideo && (
                  <AudioPlayer
                    sessionActive={active && !paused}
                    sessionDuration={duration}
                    selectedTherapy={therapy}
                    onAudioComplete={stop}
